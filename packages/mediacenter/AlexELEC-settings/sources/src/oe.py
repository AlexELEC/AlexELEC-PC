...
import oeWindows
xbmc.log('## AlexELEC Addon ## ' + unicode(__addon__.getAddonInfo('version')))
...
class ProgressDialog:
    ...
    def open(self, heading='AlexELEC', line1='', line2='', line3=''):
...
def _(code):
    wizardComp = read_setting('alexelec', 'wizard_completed')
    if wizardComp == "True":
...
def dbg_log(source, text, level=4):
    if level == 0 and os.environ.get('DEBUG', 'no') == 'no':
        return
    xbmc.log('## AlexELEC Addon ## ' + source + ' ## ' + text, level)
    if level == 4:
    ...
    try:
        for strModule in dictModules:
            ...
            if hasattr(module, 'stop_service') and module.ENABLED:
                module.stop_service()
        xbmc.log('## AlexELEC Addon ## STOP SERVICE DONE !')
    except Exception, e:
...
def load_config():
    try:
        ...
        if config_text == '':
            ...
            xml_main = xml_conf.createElement('alexelec')
...
def write_setting(module, setting, value, main_node='settings'):
    try:
        ...
        if len(xml_settings) == 0:
            for xml_main in xml_conf.getElementsByTagName('alexelec'):
...
def load_modules():
  ...
  # # load alexelec configuration modules
...
def reboot_counter(seconds=10, title=' '):
    ...
    reboot_dlg.create('AlexELEC %s' % title, ' ', ' ', ' ')
    reboot_dlg.update(0)
    wait_time = seconds
...
try:
    configFile = '%s/userdata/addon_data/service.alexelec.settings/oe_settings.xml' % XBMC_USER_HOME
    if not os.path.exists('%s/userdata/addon_data/service.alexelec.settings' % XBMC_USER_HOME):
        os.makedirs('%s/userdata/addon_data/service.alexelec.settings' % XBMC_USER_HOME)
